	...
	public void setupAnim(PlayerRenderState renderState, CallbackInfo ci) {
		...
		if (!master.equals("rust")) {
			if (!RustModPlayerAnimationAPI.animations.isEmpty())
				RustModPlayerAnimationAPI.animations.clear();
			return;
		}
		Player player = (Player) renderState.getRenderData(RustModPlayerAnimationAPI.ClientAttachments.PLAYER);
		if (player == null)
		...
		if (data.getBooleanOr("ResetPlayerAnimation", false)) {
			...
			RustModPlayerAnimationAPI.active_animations.put(player, null);
		}
		...
		RustModPlayerAnimationAPI.PlayerAnimation animation = RustModPlayerAnimationAPI.active_animations.get(player);
		if (animation == null) {
			animation = RustModPlayerAnimationAPI.animations.get(playingAnimation);
			if (animation == null) {
				RustMod.LOGGER.info("Attepted to play null animation " + playingAnimation + ", did animations fail to load?");
				return;
			}
			RustModPlayerAnimationAPI.active_animations.put(player, animation);
		}
		...
		for (Map.Entry<String, RustModPlayerAnimationAPI.PlayerBone> entry : animation.bones.entrySet()) {
			String boneName = entry.getKey();
			RustModPlayerAnimationAPI.PlayerBone bone = entry.getValue();
